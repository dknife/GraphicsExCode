...
void drawCircle(float radius) {
    ...
    for (int i=0; i<50; i++) {
        glVertex3f(radius*cos(6.28*float(i)/50.0),
                   0.0,
...
void display() {
    ...
    drawCircle(3.0); // mercury orbit
    drawCircle(5.0); // venus orbit
    drawCircle(7.0); // earth orbit
    drawCircle(10.0);// mars orbit
    drawCircle(14.0);// jupiter orbit
    ...
    drawCircle(0.7); // orbits of
    drawCircle(0.9); // the moons of
    drawCircle(1.1); // the jupiter
    glPushMatrix();
    glRotated(t*10.0, 0.0, 1.0, 0.0);
